...
public class Shi extends Chess {
	...
	public Shi(int camp, Point point) {
		super(camp, point);
	}
	...
	public boolean run(Point point, Map<Point, Chess> chess) {
		...
		int n2 = point.n;
		// 基本走法
		boolean flag = (Math.abs(n1 - n2) == 1) && (Math.abs(m1 - m2) == 1);
	...
	Image shi1 = GameUtil.getImage("Image\\shi1.png");
	Image shi2 = GameUtil.getImage("Image\\shi2.png");

	@Override
	public void draw(Graphics g) {
		if (camp > 0) {
			g.drawImage(shi1, (int) (point.x - ChessFrame.size / 2),
					(int) (point.y - ChessFrame.size / 2), ChessFrame.size,
					ChessFrame.size, null);
		} else {
			g.drawImage(shi2, (int) (point.x - ChessFrame.size / 2),
					(int) (point.y - ChessFrame.size / 2), ChessFrame.size,
					ChessFrame.size, null);
		}
	}
